...
{
    ifstream fin;
    fin.open("user_info/user_ids.txt", ios::in);
    string line;
    cout << "Username:";
    string id;
    cin >> id;
    while (getline(fin, line))
    {
        ...
        if (id == line)
        {
            ...
            cin >> pwd;
            ifstream fin;
            fin.open("user_info/user_pwds.txt", ios::in);
            while (getline(fin, line))
            {
                if (pwd == line)
                {
...
{
    ofstream fout1;
    fout1.open("user_info/user_ids.txt", ios::app);

    ofstream fout2;
    fout2.open("user_info/user_pwds.txt", ios::app);
...
void StartUp::Messages()
{
    ...
    std::cout.fill('\n');
    std::cout.width(30);
    std::cout << "Welcome to the Study Portal\n";
    std::cout.width(40);
    std::cout.fill('\n');
